...
class Client_model extends CI_Model{
    ...
    **/
    ...
    public function select_by_user_id($id){
        $this->db->select("tbl_app_users.email, $this->_table.*");
    ...
    **/
    ...
    {
        $this->db->select("$this->_table.*,tbl_app_users.*");
        $this->db->from($this->_table);
        $this->db->join("tbl_app_users", "$this->_table.user_id=tbl_app_users.user_id");
        $this->db->where("tbl_app_users.user_type", 1);
        $result = $this->db->get();
        $result = $result->result();
        return $result;
    }

    public function count_by_client(){
        $this->db->from($this->_table);
